...
let currentStaffId = null, currentStaffName = null;
...
function showPopup(title, message, success = null) {
  ...
  popupMessage.innerHTML += success === true ? '<div class="popup-icon success">Success</div>'
    : success === false ? '<div class="popup-icon error">Error</div>' : '';
  ...
  popupTimeout = setTimeout(() => { popupEl.classList.remove('show'); popupEl.style.display = 'none'; if (title === 'Location Error' && success === false) locationErrorShown = true; }, 5000);
  document.getElementById('popupCloseBtn').onclick = () => { popupEl.classList.remove('show'); popupEl.style.display = 'none'; if (title === 'Location Error' && success === false) locationErrorShown = true; };
}

function showLoader(text = "Verifying...") { loaderEl = document.getElementById("loaderOverlay"); if (loaderEl) { loaderEl.querySelector("p").textContent = text; loaderEl.style.display = "flex"; } }
...
async function fetchLocations() {
  try {
    ...
    locations = data.locations.map(l => ({ name: l.name, lat: Number(l.lat), long: Number(l.long), radiusMeters: Number(l.radiusMeters) }));
    hideLoader(); return true;
  } catch (e) { hideLoader(); showPopup('Location Error', `Failed to load locations: ${e.message}`, false); return false; }
}
...
function startLocationWatch() {
  ...
  videoEl = document.getElementById('video'); canvasEl = document.getElementById('canvas');
  popupEl = document.getElementById('popup'); popupHeader = document.getElementById('popupHeader');
  popupMessage = document.getElementById('popupMessage'); popupFooter = document.getElementById('popupFooter');
  ...
  fetchLocations().then(ok => {
    ...
    if (!navigator.geolocation) { statusEl.textContent = 'Geolocation not supported.'; clockInBtn.disabled = clockOutBtn.disabled = true; showPopup('Geolocation Error', 'Browser doesn’t support geolocation.', false); return; }
    ...
    watchId = navigator.geolocation.watchPosition(
      ...
      err => { statusEl.textContent = `Location error: ${err.message}`; clockInBtn.disabled = clockOutBtn.disabled = true; if (!locationErrorShown) showPopup('Location Error', `GPS failed: ${err.message}`, false); },
    ...
    staffIdInput.addEventListener('input', updateButtons); updateButtons();
    ...
    document.getElementById('adminDashboard').addEventListener('click', () => {
      ...
      if (adminPopup) { adminPopup.classList.add('show'); document.getElementById('adminError').textContent = ""; document.getElementById('adminEmail').value = ""; document.getElementById('adminPassword').value = ""; }
    });
  });
}

async function startVideo() {
  try { const stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: 'user' } }); videoEl.srcObject = stream; videoEl.style.transform = 'scaleX(-1)'; await videoEl.play(); return true; }
  catch (err) { showPopup('Camera Error', `Access denied: ${err.message}`, false); return false; }
}
...
async function validateFaceWithProxyTargeted(base64, targetSubject) {
  try {
    const res = await fetch('/api/proxy/face-recognition', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ file: base64, subject: targetSubject }) });
...
async function handleClock(action) {
  ...
  currentStaffName = staffName; hideLoader();
  ...
  let office = null;
  for (const loc of locations) { if (getDistanceKm(lat, long, loc.lat, loc.long) <= loc.radiusMeters / 1000) { office = loc.name; break; } }
  ...
  const tempCanvas = document.createElement('canvas'); tempCanvas.width = 640; tempCanvas.height = 480;
  ...
  if (!faceRes.ok) {
    hideLoader();
    if (faceRes.noSubject) showPopup('Face Not Registered', `Dear ${staffName}, your face has not been added. See HR.`, false);
    else showPopup('Face Error', faceRes.error || 'Verification failed.', false);
    return;
  }
  if (faceRes.similarity < 0.9) { hideLoader(); showPopup('Face Error', 'Face not clear. Try better lighting.', false); return; }
...
async function fetchAdminLogins() {
  try { showLoader('Logging in...'); const res = await fetch('/api/admin-logins', { mode: 'cors' }); if (!res.ok) throw new Error(`HTTP ${res.status}`); const data = await res.json(); hideLoader(); return data.success ? data.logins : []; }
  catch (e) { hideLoader(); showPopup('Admin Error', `Failed to fetch logins: ${e.message}`, false); return []; }
}
function loginAdmin() {
  const email = document.getElementById('adminEmail').value.trim(), password = document.getElementById('adminPassword').value.trim();
  const adminError = document.getElementById('adminError'), adminPopup = document.getElementById('adminPopup');
  ...
  fetchAdminLogins().then(logins => {
    ...
    if (valid) { localStorage.setItem('isLoggedIn', 'true'); localStorage.setItem('lastActivity', Date.now()); adminPopup.classList.remove('show'); window.location.href = 'stats.html'; }
    else adminError.textContent = 'Invalid email or password.';
  });
}

document.addEventListener('DOMContentLoaded', () => {
  const adminPopup = document.getElementById('adminPopup');
  if (adminPopup) adminPopup.addEventListener('click', e => { if (e.target === adminPopup) adminPopup.classList.remove('show'); });
  startLocationWatch(); initSessionTimeout();
});
window.onunload = () => { if (watchId) navigator.geolocation.clearWatch(watchId); stopVideo(); };

function initSessionTimeout() {
  let timeoutId; const SESSION_TIMEOUT = 86400000;
  const isLoggedIn = () => localStorage.getItem('isLoggedIn') === 'true';
  const logout = () => { localStorage.removeItem('isLoggedIn'); localStorage.removeItem('lastActivity'); clearTimeout(timeoutId); window.location.href = 'index.html'; };
  const resetTimeout = () => {
