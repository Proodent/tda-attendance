...
// === UTILITY ===
function toRad(v) { return v * Math.PI / 180; }let watchId = null;
let currentOffice = null;
let staffCache = new Map();
let videoEl, faceModal, captureStatus;
let countdown = 0;
...
function startLocationWatch() {
  ...
  // === FORCE VALIDATION BOX TO STAY VISIBLE IF TEXT EXISTS ===
  const ensureValidationVisible = () => {
  ...
  const triggerValidation = () => {
    ...
    requestAnimationFrame(() => updateUserStatus());
  };

  // === ATTACH ALL INPUT EVENTS ===
  ['input', 'keydown', 'keyup', 'paste', 'cut', 'change', 'blur'].forEach(event => {
    userIdInput.addEventListener(event, triggerValidation);
  });
  ...
  // === BULLETPROOF VALIDATION – NEVER HIDES UNTIL CLEARED ===
  async function updateUserStatus() {
    ...
    // FORCE BOX TO STAY VISIBLE
    userIdStatus.classList.add('show');

    // Only validate if ID changed
