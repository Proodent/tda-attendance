...
// ----------------- Attendance Logging — TIME OUT NOW BEHAVES EXACTLY LIKE TIME IN -----------------
app.post("/api/attendance/web", async (req, res) => {
  try {
    ...
    // SAVE TIME AS REAL TIME VALUE (fraction of day) — BOTH IN AND OUT BEHAVE IDENTICALLY
    const timeFraction = dt.getHours() / 24 + dt.getMinutes() / (24*60) + dt.getSeconds() / (24*60*60);
    ...
    if (action === "clock in") {
      ...
      await attendanceSheet.addRow({
        ...
        "Name": subjectName,
        "UserID": userIdStr,
        "Time In": timeFraction,   // ← REAL TIME VALUE
        "Clock In Location": officeName,
        "Time Out": "",
        "Clock Out Location": ""
      });

      return res.json({ success: true, message: `Dear ${subjectName}, clock-in recorded at ${dt.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', second: '2-digit' })} (${officeName}).` });
    }
    ...
    if (action === "clock out") {
      ...
      existing["Time Out"] = timeFraction;  // ← REAL TIME VALUE — SAME AS TIME IN
      existing["Clock Out Location"] = officeName;
      await existing.save();

      return res.json({
        success: true,
        message: `Dear ${subjectName}, clock-out recorded at ${dt.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', second: '2-digit' })} (${officeName}).`
      });
    }
