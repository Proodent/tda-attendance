...
// ----------------- Attendance Logging (FIXED: Use UserID for unique check, prevent multiples) -----------------
app.post("/api/attendance/web", async (req, res) => {
  try {
    ...
    // FIXED: Find existing by UserID and Date (unique, no name mismatch)
    const existing = attendanceRows.find(r =>
      (r["Date"] || r.get("Date")) === dateStr &&
      (r["UserID"] || r.get("UserID") || "").toString().trim() === userIdStr
    );
    if (action === "clock in") {
    ...
    if (action === "clock out") {
      ...
      if (!existing["Time In"] || !existing.get("Time In")) {
        return res.json({ success: false, message: `Dear ${subjectName}, no clock-in found for today.` });
      }
